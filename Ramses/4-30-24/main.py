...
class Widget1():
    ...
    def button1_open(self):
        ...
        self.open_file(file_path, file_extension)
    ...
    def open_file(self, file_path, file_extension):
        if "-Index" in file_path:
            original_filename, original_extension = os.path.splitext(file_path)
            file_split = original_filename.split('-Index-')
            file_extension = ("."+file_split[1])
            file_path = (file_split[0]+file_extension)
        if file_extension.lower() == ".txt":
            self.open_text_file(file_path)
        elif file_extension.lower() in [".jpg", ".jpeg", ".png", ".gif"]:
            self.open_image_file(file_path)
        elif file_extension.lower() in [".docx", ".doc", ".docm", ".dot", ".dotm", ".dotx", ".htm", ".html", ".mht",
                                        ".mhtml", ".odt", ".rtf"]:
            self.open_word_file(file_path)
        elif file_extension.lower() in [".pdf"]:
            self.open_pdf_file(file_path)
        elif file_extension.lower() in [
        ".pptx", ".pptm", ".pdf", ".xps", ".potx", ".potm", ".thmx", ".ppsx", ".ppsm", ".ppam", ".xml", ".mp4", ".wmv", ".gif", ".jpg", ".png", ".tif", ".bmp", ".wmf", ".emf", ".odp",]:
            self.open_powerpoint_file(file_path)
        else:
            self.error_message()
    ...
    def open_image_file(self,file_path):
        try:
            # Open the image file using the PIL Image class (renamed to PILImage)
            ...
            messagebox.showerror("Error", f"An error reading the file")
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {e}")
    ...
    def open_powerpoint_file(self, file_path):
        try:
            # Use subprocess to open the file with the default application
            if os.name == 'nt':  # For Windows
                os.startfile(file_path)
    ...
    def new_keyword_func(self):
        ...
        if self.file_path:
            if "-Index" in self.file_path.name:
                messagebox.showerror('Python Error', 'Error: Cannot make an Index file for an Index file')
            else:
    ...
    def create_index(self):
        if hasattr(self, 'file_path'):
            ...
            original_filename, original_extension = os.path.splitext(original_path)
            index_path = f"{original_filename}-Index{original_extension.replace('.', '-')}.txt"
            try:
